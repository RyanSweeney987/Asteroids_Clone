...
*/
// comment the line below if you get an error.. 
//#include "stdafx.h"
...
void ScreenText::createTextSurface(void)
{
	...
	//SDL_Surface * textSurface = TTF_RenderText_Shaded(fontptr, currenttext.c_str(), color, backgroundcolour);
	SDL_Surface* textSurface = TTF_RenderText_Blended(fontptr, currenttext.c_str(), color);
...
void ScreenText::setText(const std::string& text, const int posX, const int posY, const bool bCenterText, const int fontSize, const int r, const int g, const int b)
{
	...
	if (bCenterText)
	{
		this->x = posX - (int)(width * 0.5f);
		this->y = posY - (int)(height * 0.5f);
	}
